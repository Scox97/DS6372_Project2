...
authors: "Adam Ercanbrack, Joel Laskow, Steven Cox"
date: "`r Sys.Date()`"
...
# Loading in the Data
```{r load in adult dataset, echo=FALSE}
adult<-read.csv("50_or_More.csv", stringsAsFactors = TRUE)
...
**   Findings: Of 32562 data values 4262 are missing (less than 1 percent missing).

## Visualizing Missing Values
```{r Visualize_missing_values}
#Using adult_temp to check missing so we don't have to change all the relabeling code
# Convert '?' to NA in the adult dataframe
adult_missing <- adult


adult_missing <- data.frame(lapply(adult_missing, function(x) {
  # Check if the column is of character type
  if (is.character(x)) {as.factor(trimws(x))} else {x}}))

adult_missing[] <- lapply(adult_missing, function(x) {
  if (is.character(x)) {
    # Replace '?' with NA in character columns
    x[x == "?"] <- NA
    x
  } else if (is.factor(x)) {
    # Replace '?' with NA in factor columns, after converting to character
...
str(adult_missing)
cat("\n------------------Count NA----------------------\n\n")
sort(colSums(is.na(adult_missing)), decreasing = TRUE)
vis_miss(adult_missing)


adult_categorical <- adult_missing %>% select_if(~!is.numeric(.))
missing_categorical <- vis_miss(adult_categorical)
missing_categorical + ggtitle("Missing Data (Categorical)") + theme(plot.title = element_text(hjust = 0.5))

adult_numeric <- adult_missing %>% select_if(is.numeric)
adult_numeric[adult_numeric == ""] <- NA
missing_numeric <- vis_miss(adult_numeric)
missing_numeric + ggtitle("Missing Data (Numeric)") + theme(plot.title = element_text(hjust = 0.5))

# Summary Missing Data
total_missing <- sum(sapply(adult_missing, function(x) sum(is.na(x))))
print(total_missing)

# Remove temporary data frames to save resources
remove(adult_missing)
remove(adult_categorical)
remove(missing_categorical)
remove(adult_numeric)
remove(missing_numeric)

```

## Reclassifing Categorical Variables and addressing '?':
...
adult$native_region <- factor(adult$native_region, levels = names(new_levels))
...
# Create invested variable based on whether observation has a value in capital_gain or capital_loss
adult <- adult %>%
...
```{r loess_gender}
# Check the continuous variables for linearity against the log odds of Y
levels(adult$gender)
...
adult$income.num<-ifelse(adult$income==">50K", 1, 0)

# Calculate proportions
positive_proportions2 <- table(adult$native_region, adult$income.num)[, "1"] / rowSums(table(adult$native_region, adult$income.num))
...
heatmap_data <- data.frame(native_region = names(positive_proportions), positive_proportion = positive_proportions)

heatmap_data$native_region <- factor(heatmap_data$native_region, levels = heatmap_data$native_region[order(heatmap_data$positive_proportion, decreasing = FALSE)])


ggplot(heatmap_data, aes(x = "", y = native_region, fill = positive_proportion)) +
  geom_tile() +
  scale_fill_gradient(low = "white", high = "blue") +
  theme_minimal() +
  labs(title = "Proportion of Positive Income Status by native_region of Origin", x = NULL, y = NULL) +
...
View(adult2)
model <- glm(factor(income) ~ ., data = adult2, family = binomial)
...
native_regionSouthAmerica: An OR of 0.19791245 suggests that being from South America is associated with an 80.21% decrease in the odds of the outcome occurring compared to not being from South America.
...
# Logging Capital
## Train
capital_data_training<-training_set[,c("capital_gain","capital_loss")]
...
training_set<-cbind(training_set, capital_data_training)

# Assigning classes in transformed dataset

training_set$age<-as.numeric(training_set$age)
training_set$workclass<-factor(training_set$workclass)
training_set$education_level<-factor(training_set$education_level)
training_set$years_education<-as.numeric(training_set$years_education)
training_set$marital_status<-factor(training_set$marital_status)
training_set$occupation<-factor(training_set$occupation)
training_set$household_role<-factor(training_set$household_role)
training_set$continent<-factor(training_set$continent)
training_set$log_capital_loss<-as.numeric(training_set$log_capital_loss)
training_set$log_capital_gain<-as.numeric(training_set$log_capital_gain)


## Test

capital_data_testing <- testing_set[, c("capital_gain", "capital_loss")]
...
capital_data_testing <- capital_data_testing %>%
  ...
  mutate(
    log_capital_gain = log(capital_gain),
    log_capital_loss = log(capital_loss)
  )


testing_set <- cbind(testing_set, capital_data_testing)

# Assigning classes in transformed dataset
testing_set$age <- as.numeric(testing_set$age)
testing_set$workclass <- factor(testing_set$workclass)
testing_set$education_level <- factor(testing_set$education_level)
testing_set$years_education <- as.numeric(testing_set$years_education)
testing_set$marital_status <- factor(testing_set$marital_status)
testing_set$occupation <- factor(testing_set$occupation)
testing_set$household_role <- factor(testing_set$household_role)
testing_set$continent <- factor(testing_set$continent)
testing_set$log_capital_loss <- as.numeric(testing_set$log_capital_loss)
testing_set$log_capital_gain <- as.numeric(testing_set$log_capital_gain)



glm_final<-glm(income.num~age+workclass+education_level+years_education+marital_status+occupation+household_role+race+continent+log_capital_loss+log_capital_gain, data=training_set, family="binomial")
...
predicted_probs_final <- predict(glm_final, newdata = testing_set, type = "response")

testing_set$income.num <- ifelse(testing_set$income == ">50K", 1, 0)


roc_curve_final <- roc(testing_set$income, predicted_probs_final)
...
confusion_matrix_final <- confusionMatrix(result_df$Predicted_Class, factor(testing_set$income.num))
...
glm_final<-glm(income.num~age+years_education+marital_status+occupation+household_role+gender+race+log_capital_loss+log_capital_gain, data=training_set, family="binomial")
...
confusion_matrix_final_reduced_with_outliers <- confusionMatrix(result_df$Predicted_Class, factor(testing_set$income.num))
...
training_set_outliers_removed <- training_set[-outlier_indices, ]

outlier_indices <- c(4569, 5386, 10459, 15377, 17040, 18437, 20177, 31369, 32215, 32562)
testing_set_outliers_removed <- testing_set[-outlier_indices, ]



glm_final_reduced_without_outliers<-glm(income.num~age+years_education+marital_status+occupation+household_role+gender+race+log_capital_loss+log_capital_gain, data=training_set_outliers_removed, family="binomial")
...
print_model_metrics <-
  function(model, test_data, response_var, pos_value, threshold, model_title) {

    # Determine the positive and negative values
    # pos_value is provided as an argument
    neg_value <- setdiff(levels(test_data[[response_var]]), pos_value)
    ...
    predicted.classes <- ifelse(probabilities >= threshold, pos_value, neg_value)
    

    actual.classes    <- factor(test_data[[response_var]], levels = c(pos_value, neg_value))
    predicted.classes <- factor(predicted.classes, levels = c(pos_value, neg_value))
    
    cm <- confusionMatrix(predicted.classes, actual.classes, positive = pos_value)
    print(cm)
    ...
    metrics_values <-
      c(accuracy, sensitivity, specificity, prevalence, ppv, npv, auroc)
    
    metrics_values[is.na(metrics_values)] <- 0
    metrics_values <- metrics_values * 100
    metrics_values <- paste(round(metrics_values, 2), "%", sep="")
    
    # Compile the metrics into a data frame
    metrics <- data.frame(
      Metric = c("Accuracy","Sensitivity","Specificity",
                 "Prevalence","PPV","NPV","AUROC"),
...
plot_model_metrics_by_threshold <-
  function(model,
    ...
    # Find the threshold where sensitivity and specificity intersect
    results$Difference = abs(results$Sensitivity - results$Specificity)
    best_threshold = results$Threshold[which.min(results$Difference)]
    
 # Melt results for plotting, excluding 'Difference'
    results_long <-
      reshape2::melt(results[,-8], id.vars = 'Threshold', variable.name = 'Metric')
    
    # Plot results
    plot <-
      ggplot(results_long, aes(x = Threshold, y = value, color = Metric)) +
      geom_line() +
      geom_vline(xintercept = best_threshold, linetype = "dashed", color = "black", size = 1) +
      annotate("text", x = best_threshold, y = 0.05, label = sprintf("Best Threshold: %.2f", best_threshold), hjust = 0) +
      labs(title = model_title, x = "Threshold", y = "Metric Value") +
      scale_y_continuous(limits = c(0, 1)) +
      ggtitle(paste("Metrics for Various Thresholds -", model_title)) 
      scale_color_manual(
        values = c("red", "blue", "purple", "cyan", "orange", "green"),
        labels = c(
...
model_formula <- as.formula("income ~ age * gender  +
                                      hours_per_week * race +
                                      years_education +
                                      capital_gain_log +
                                      capital_loss_log +
...
model_formula <- as.formula("income ~ poly(age,2)*gender +
                                      poly(years_education,2) * marital_status +
                                      poly(hours_per_week,2) * marital_status +
                                      poly(capital_gain_log,2) +
                                      poly(capital_loss_log,2) +
                                      household_role +
                                      occupation")
...
model_formula <- as.formula("income ~ age * native_region + 
                                      years_education * native_region + 
                                      hours_per_week * native_region +
                                      capital_gain_log * native_region +
...
model_formula <- as.formula("income ~ age * native_region + 
                                      ...
                                      hours_per_week +
                                      occupation +
                                      poly(capital_gain_log,2) * native_region +
...
model_formula <- as.formula("income ~ age * race + 
                                      years_education * race + 
                                      marital_status * race + 
                                      household_role * race + 
                                      hours_per_week +
                                      poly(capital_gain_log, 2)+
...
```{r, warning=FALSE}
# Using Race as interaction except for capital_gain_log and capital_loss_log which is using invested
model_formula <- as.formula("income ~ (age * occupation +
                                      years_education +
...
interactions_formula <-
  as.formula(
    income ~ +age + workclass_num + education_level_num + years_education + marital_status_num +
